...
test("fillMissingStepTimes 用真实语音分段补齐缺失时间并保留已有时间", () => {
  ...
  assert.ok(steps.every((step) => Number.isFinite(step._sourceImageTime)));
  assert.ok(!JSON.stringify(steps).includes("_sourceImageTime"));
...
test("extractStepImages 单候选帧也经视觉复核，不合适则不配图", async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "paoding-step-test-"));
  ...
  try {
    ...
    globalThis.fetch = async (input, init = {}) => {
      ...
      if (system.includes("挑选步骤配图")) {
        checks++;
        return respond(JSON.stringify({ ok: !userText.includes("黑屏转场") }));
      }
      return respond("{}");
    };
    ...
    const saved = await extractStepImages({
      ...
      apiKey: "test-key",
      model: "vision-stub",
    }, videoPath, recipe, { duration: 120, imagesDir });
...
test("extractIngredientImages 允许将食材图定位并发降到 1", async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "paoding-ing-serial-test-"));
  const oldFetch = globalThis.fetch;
  const oldFfmpeg = process.env.PAODING_FFMPEG_BIN;
  let activeCrops = 0, maxActiveCrops = 0;
  const respond = (content) => new Response(JSON.stringify({
  ...
  try {
    ...
    globalThis.fetch = async (input, init = {}) => {
      ...
      const body = JSON.parse(String(init.body || "{}"));
      const system = String(body.messages?.find((m) => m.role === "system")?.content || "");
      if (system.includes("给食材清单配图")) return respond(JSON.stringify({ 鸡蛋: 1, 番茄: 1 }));
      if (system.includes("视觉定位助手")) {
        activeCrops++;
        maxActiveCrops = Math.max(maxActiveCrops, activeCrops);
        try {
    ...
    fs.writeFileSync(videoPath, "fake video\n");
    fs.mkdirSync(imagesDir);
    const recipe = { ingredients: [{ name: "鸡蛋" }, { name: "番茄" }] };

    const saved = await extractIngredientImages({
      baseUrl: "http://vision.test/v1",
      apiKey: "test-key",
      model: "vision-stub",
      ingredientConcurrency: 1,
    }, videoPath, recipe, { duration: 20, imagesDir });

    assert.equal(saved, 2);
    assert.equal(maxActiveCrops, 1);
  } finally {
    globalThis.fetch = oldFetch;
    if (oldFfmpeg === undefined) delete process.env.PAODING_FFMPEG_BIN;
    else process.env.PAODING_FFMPEG_BIN = oldFfmpeg;

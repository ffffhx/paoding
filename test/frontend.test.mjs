...
test("i18n zh/en 字典键集合保持一致", () => {
  const zhKeys = Object.keys(app.PaodingI18n.zh).filter(k => !k.startsWith("test.")).sort();
  const enKeys = Object.keys(app.PaodingI18n.en).filter(k => !k.startsWith("test.")).sort();
...
test("keyboardShortcutAction 映射桌面快捷键且输入框聚焦不劫持", () => {
  ...
  assert.equal(app.keyboardShortcutAction({ key: " ", isComposing: true, target: { tagName: "BODY" } }, { cookMode: true }), "");
  assert.equal(app.keyboardShortcutAction({ key: "Process", target: { tagName: "BODY" } }, { cookMode: true }), "");
  assert.equal(app.keyboardShortcutAction({ key: "r", ctrlKey: true, target: { tagName: "BODY" } }, { cookMode: true }), "");
  assert.equal(app.keyboardShortcutAction({ key: " ", target: { tagName: "SPAN", getAttribute: (name) => name === "role" ? "button" : "" } }, { cookMode: true }), "");
});
...
test("computeKitchenStats 按记录时区处理跨月 streak 和日期型老记录", () => {
  const stats = app.computeKitchenStats({
    recipes: [
      { id: "jp", title: "跨时区饭" },
      { id: "plain", title: "日期老记录" },
    ],
    meta: {
      jp: {
        cooked: true,
        cooked_history: [
          "2026-02-27T23:30:00+09:00",
          "2026-02-28T23:30:00+09:00",
        ],
        cooked_at: "2026-03-01T00:30:00+09:00",
      },
      plain: { cooked: true, cooked_dates: ["2026-03-01"] },
    },
  }, { now: "2026-03-01T01:00:00+08:00" });
  assert.equal(stats.weekCount, 4);
  assert.equal(stats.monthCount, 2);
  assert.equal(stats.streakDays, 3);
  assert.deepEqual(Array.from(stats.topRecipes.map(r => `${r.title}:${r.count}`)), ["跨时区饭:3", "日期老记录:1"]);
});

test("computeKitchenStats 显式取消做过时不再把旧 cooked_at 计入统计", () => {
  const stats = app.computeKitchenStats({
    recipes: [{ id: "a", title: "取消标记" }],
    meta: { a: { cooked: false, cooked_at: "2026-03-01T08:00:00+08:00" } },
  }, { now: "2026-03-01T12:00:00+08:00" });
  assert.equal(stats.weekCount, 0);
  assert.equal(stats.monthCount, 0);
  assert.equal(stats.cookedRecipeCount, 0);
...
test("原视频片段控件优先使用应用内媒体，旧菜谱才降级为外链", () => {
  ...
  assert.ok(app.recipeMediaUrl({ id: "蛋", source_media: "source.mp4" }, "step-4.mp4").includes("/media/step-4.mp4"));

  const clipped = app.sourceSegmentControlHtml({ id: "蛋", source: "http://xhslink.com/a" }, { ...step, source_clip: "step-4.mp4" });
  assert.ok(clipped.includes("<button"));
  assert.ok(clipped.includes("看原视频这一段"));
  ...
  const fullVideo = app.sourceSegmentControlHtml({ id: "蛋", source: "http://xhslink.com/a", source_media: "source.mp4" }, { index: 5 });
  assert.ok(fullVideo.includes("<button"));
  assert.ok(fullVideo.includes('data-source-segment="5"'));
  assert.ok(fullVideo.includes("看原视频"));
  assert.ok(!fullVideo.includes("这一段"));
  assert.equal(app.sourceSegmentControlHtml({ source: "http://xhslink.com/a" }, { index: 5 }), "");
});

test("菜谱简介页看原视频优先在庖丁内播放，旧菜谱才跳原平台", () => {
  const internal = app.sourceOverviewControlHtml({
    id: "蛋",
    source: "http://xhslink.com/a",
    source_media: "source.mp4",
  });
  assert.ok(internal.includes("<button"));
  assert.ok(internal.includes("data-source-full"));
  assert.ok(!internal.includes("xhslink.com"));

  const fallback = app.sourceOverviewControlHtml({ source: "http://xhslink.com/a" });
  assert.ok(fallback.includes("<a"));
  assert.ok(fallback.includes("xhslink.com/a"));
  assert.equal(app.sourceOverviewControlHtml({ source: "（粘贴文字）" }), "");
});
...
test("recipeShareCardLayout 按显示宽度截断中英混排并清理下载文件名", () => {
  const layout = app.recipeShareCardLayout({
    id: "mixed",
    title: "麻婆 tofu bourguignon 超级浓郁番茄牛腩 with extra-long western title".repeat(2),
    ingredients: [
      { name: "超级超级超级超级超级超级长的ingredient-name", amount: "1234567890克" },
      { name: "fresh basil 罗勒叶", amount: "a very generous handful" },
    ],
    steps: [{ title: "Prep", action: "把 very long English explanation 和中文说明混在一起" }],
  }, 1);
  assert.ok(layout.title.lines.every(line => app.shareTextWidthUnits(line) <= layout.title.maxLineUnits));
  const ingredientLines = Array.from(layout.ingredientSection.columns[0]).concat(Array.from(layout.ingredientSection.columns[1]));
  assert.ok(ingredientLines.every(line => app.shareTextWidthUnits(line) <= layout.ingredientSection.maxLineUnits));

  const stem = app.safeDownloadStem("../<img src=x onerror=alert(1)>\u202Egnp.exe\n");
  assert.ok(!/[\/\\:*?"<>|\u0000-\u001f\u007f-\u009f\u202a-\u202e\u2066-\u2069]/.test(stem));
  assert.ok(!stem.includes(".."));
  assert.equal(app.safeDownloadStem("CON"), "recipe-CON");
});

...
test("explainSteps 超过 4 步时按批生成并带相邻上下文", async () => {
  const payloads = [];
  const llm = await startLlmStub({
    explanations: (body) => {
      const user = String(body.messages?.find((m) => m.role === "user")?.content || "");
      const payload = JSON.parse(user.replace(/^菜谱讲解输入：\n/, ""));
      payloads.push(payload);
      return {
        explanations: payload.steps_to_explain.map((s) => ({
          index: s.index,
          reason: `解释 ${s.index}`,
          if_not: `后果 ${s.index}`,
          cue: `判断 ${s.index}`,
  ...
  try {
    const recipe = {
      ...
      servings: "2人份",
      cuisine: "家常菜",
      ingredients: [{ name: "鸡蛋", amount: "2个", note: "视频明确" }],
    ...
    const batches = payloads
      .map((p) => p.steps_to_explain.map((s) => s.index))
      .sort((a, b) => a[0] - b[0]);
    assert.deepEqual(batches, [[1, 2, 3, 4], [5]]);
    const first = payloads.find((p) => p.steps_to_explain[0].index === 1);
    const second = payloads.find((p) => p.steps_to_explain[0].index === 5);
    assert.deepEqual(first.context_steps.map((s) => s.index), [5]);
    assert.deepEqual(second.context_steps.map((s) => s.index), [4]);
    assert.equal(first.recipe.ingredients[0].name, "鸡蛋");

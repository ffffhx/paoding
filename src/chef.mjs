...
export const TOOL_PROMPT_RULES = `   - 甜品/烘焙类（蛋糕、饼干、慕斯、塔派、面包、裱花、巧克力、糖艺等，由菜品和步骤判断）必须完整列出关键工具：打发器/打蛋器、裱花袋和裱花嘴、抹刀/刮刀、模具（如视频或菜谱提到尺寸要写明）、油纸、厨房秤、温度计、烤箱等。
   - 每个工具都必须说明 purpose；有替代品时 substitute 写替代方案，substitute_note 写代价/注意点；没有替代品时 substitute 必须为 null，substitute_note 只写原因本身，不要复述「无替代/没有替代方案/因为」这类结论或开头。
   - 替代判断要务实：不要把“有代价/不够完美”当成“无替代”。常见例子：电动打蛋器→手动打蛋器（耗时费力且稳定性更差）；裱花袋/裱花嘴→保鲜袋剪小口（线条粗细和稳定性较差，若只是装饰步骤可跳过或改粗略抹面）；筛网→细目滤网；刮刀→饭勺或硅胶铲。
   - 只有当工具决定结构、温度或成型且普通替代会明显失败时才判定无替代，如烤箱、指定尺寸/材质模具、需要架空通风的烤架等；这类 substitute 为 null，substitute_note 只写具体失败原因。
   - 非甜品只列非常规厨具（如蒸锅、砂锅、烤箱、温度计），家家都有的锅碗瓢盆不要列。
   - 视频画面、口播或菜谱步骤出现过的工具照实提取；没出现但该工艺必需的按常识推断并把 inferred 标 true；拿不准宁可不列。`;
...
2. 步骤要拆细、可执行，保留关键参数：火候、油温、时间、以及"怎么算到位"的状态判断（如"煎至两面金黄""炒出糖色"）。
   - action 必须逐句覆盖转写里影响成败的所有操作细节，不得为了简短把连续微动作概括掉。尤其要保留：动作先后、双手如何配合、边做 A 边做 B、每次少量加入、把堆积物拨到一边、静置后再捞、捞出时挤压/沥油等具体手法。
   - 同一个步骤里出现多个连续动作时，可以用分号串成完整操作，也可以拆成相邻步骤；但不能只保留开头和结果。例："慢慢倒入，边倒边搅；膨起后拨到一边，继续往空处倒；静置一分钟后捞出并压油"不得压缩成"倒入，膨胀后捞出"。
   - params 是 action 的结构化补充，不能代替 action。原文说过的时间、火候、状态和手法，action 中也要自然写明，保证用户只读操作正文就能做对。
...
13. 若标题或转写出现「N道菜」「第一道/第二道/第X道」「合集/菜单/年夜饭」等多道菜合集信号：不要只整理第一道，也不要丢弃后半段。title 可写合集标题；ingredients 汇总各道菜主要食材；steps 至少为每一道菜输出 1 个步骤（必要时每道 1-3 步），step.title 用「第X道：菜名/做法」。source_time 优先取该道开头到下一道开头，或该道关键操作区间；某道只讲储存/调汁/技巧时也照实输出对应步骤。
14. 严格输出如下 JSON（不要多余字段、不要注释、不要 markdown）：
  ...
  "tools": [
    {
      ...
      "substitute_note": "有替代时写代价/注意点；无替代时只写原因本身，不要写“无替代方案，因为…”",
      "inferred": false
    }
  ],
  "steps": [
...
function hasConcreteAmountToken(text) {
  return (String(text || "").match(AMOUNT_TOKEN_RE) || []).length > 0;
}

function pushRecipeCardBlock(out, block) {
  if (block?.some(hasConcreteAmountToken)) out.push(block.join("\n"));
}

const MULTI_DISH_MARKER_RE = /第[一二三四五六七八九十百千万零〇两0-9]+道/;
const TIMED_LINE_RE = /^\[(\d{1,2}):(\d{2})\]\s*(.*)$/;

function formatOutlineTime(sec) {
...
function parseTimedLine(line) {
  const m = String(line || "").match(TIMED_LINE_RE);
  if (!m) return null;
  return { sec: Number(m[1]) * 60 + Number(m[2]), text: m[3].trim() };
}

function cleanOutlineLine(text) {
  return String(text || "")
    .replace(MULTI_DISH_MARKER_RE, "")
    .replace(/[，。,.!！?？]+$/g, "")
    .replace(/\s+/g, " ")
    .trim();
}

export function extractMultiDishOutline(transcript) {
  return formatMultiDishOutline(extractMultiDishSegments(transcript));
}

function formatMultiDishOutline(segments) {
  return segments.map((seg) => {
    const range = seg.end ? `${formatOutlineTime(seg.start)}-${formatOutlineTime(seg.end)}` : `${formatOutlineTime(seg.start)}`;
    return `- ${seg.marker} [${range}] ${seg.summary}`;
  }).join("\n");
}

export function extractMultiDishSegments(transcript) {
  const segments = [];
  let current = null;
  let maxSec = 0;
  for (const line of String(transcript || "").split(/\r?\n/)) {
    const timed = parseTimedLine(line);
    if (!timed) continue;
    maxSec = Math.max(maxSec, timed.sec);
    const marker = timed.text.match(MULTI_DISH_MARKER_RE)?.[0];
    if (marker) {
      if (current) segments.push(current);
      current = { marker, start: timed.sec, lines: [] };
      const rest = cleanOutlineLine(timed.text);
      if (rest) current.lines.push(rest);
      continue;
    }
    if (!current || current.lines.length >= 10) continue;
    const text = cleanOutlineLine(timed.text);
  ...
  return segments.map((seg, i) => {
    ...
    const summary = seg.lines.join("，").slice(0, 140) || "仅检测到道次标记";
    return {
      marker: seg.marker,
      start: seg.start,
      end: end ?? Math.max(maxSec, seg.start + 2),
      summary,
    };
  });
}

function multiDishStepTitle(seg) {
  const first = seg.summary.split(/[，。；;,.]/).find(Boolean) || "做法";
  return `${seg.marker}：${first.slice(0, 24)}`;
}

function cleanedCollectionTitle(meta, recipe) {
  const raw = cleanText(meta?.title || recipe?.title || "多道菜合集", 120);
  return raw
    .replace(/#.*$/g, "")
    .replace(/[（(]附时间轴[）)]/g, "")
    .replace(/[|｜].*$/g, "")
...
function applyMultiDishFallback(recipe, segments, meta) {
  if (!segments.length) return recipe;
  const steps = Array.isArray(recipe.steps) ? recipe.steps : [];
  const minimum = Math.ceil(segments.length * 0.8);
  if (steps.length >= minimum) return recipe;
  recipe.title = cleanedCollectionTitle(meta, recipe);
  ...
  recipe.steps = segments.map((seg, i) => ({
    index: i + 1,
    title: multiDishStepTitle(seg),
    action: seg.summary,
    params: { heat: "", temp: "", time: "", cue: "" },
    source_time: [Math.round(seg.start), Math.max(Math.round(seg.end), Math.round(seg.start) + 2)],
  }));
  return recipe;
}
...
export async function structureRecipe(llm, { transcript, meta, signal }) {
  ...
  const multiDishSegments = extractMultiDishSegments(transcript);
  const multiDishOutline = formatMultiDishOutline(multiDishSegments);
  const cardBlock = cardTranscript ? `【画面配方卡 / 配料表（高优先级）】
以下来自视频画面中的配方卡/配料表，用量以它为准（比口播记忆更可靠），与口播冲突时优先采用，并在对应食材 note 里注明「出处：画面配方卡」。
${cardTranscript}

` : "";
  const outlineBlock = multiDishOutline ? `【多道菜目录（从时间戳自动提取，高优先级）】
以下目录显示视频中每一道菜/技巧出现的位置。结构化时必须覆盖这些条目，不能只处理第一道；每个目录项至少对应 steps 中 1 个步骤：
${multiDishOutline}

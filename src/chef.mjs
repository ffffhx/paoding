...
4. 【语音转写】来自语音识别，含同音/近音错别字，尤其是香料和食材名。整理时必须按烹饪常识纠正为正确写法（例：白纸→白芷、肉豆扣/肉豆寇→肉豆蔻、草扣→草蔻、山奈→山柰）。纠正过的食材在 note 里标注：转写作「原词」，已按烹饪常识纠正。拿不准是不是错别字时保留原词，不要乱改。
5. 若转写行首带 [分:秒] 时间标记，请为每一步额外输出 "source_time": [起始秒, 结束秒]（整数秒，该步操作在原视频中对应的时间范围）。source_time 必须直接取自转写中真实出现的时间标记，绝不能超过最后一个时间标记的秒数、绝不能凭步骤先后自行外推。转写没有时间标记时不要输出 source_time。
6. 工具/器具清单：
...
7. 用简体中文。面向中式家常菜，也能处理甜品/烘焙菜谱。
8. 严格输出如下 JSON（不要多余字段、不要注释、不要 markdown）：

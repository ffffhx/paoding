...
// signal：AbortSignal，任务超时时用来强杀卡死的子进程（spawn 会随 signal abort 杀掉 child）。
...
export async function acquire(input, ytdlp = {}, onProgress = () => {}, signal) {
  if (!(await has("ffmpeg"))) {
  ...
  if (isUrl(input)) {
    ...
    await run("yt-dlp", ["-x", "--audio-format", "mp3", "--no-playlist", ...common, "-o", tmpl, input], {
      signal,
      onData: (s) => {
        const m = s.match(/(\d+(?:\.\d+)?)%\s+of/);
        if (m) onProgress({ pct: 8 + Math.min(90, +m[1]) * 0.8, message: "下载音频…" }); // 8→80
      },
    });

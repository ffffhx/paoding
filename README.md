...
| 🔄 **生态互通** | 导入单个 **schema.org Recipe JSON-LD**；复制 Markdown、下载 **Cooklang `.cook`** 与 **schema.org JSON-LD**；公开分享页可导出 JSON-LD，方便复制到自己的庖丁实例 |
...
纯 Node 内置测试器、零第三方依赖：覆盖解析纯函数、服务端接口、前端 `app.js` 纯逻辑、schema.org 导入/导出、图片导入、技法聚合与归纳缓存、自动备份纯函数与管线集成测试。管线测试使用 `test/fixtures/bin/` 里的 fake `yt-dlp` / `ffmpeg` / `whisper-cli`，并用 OpenAI 兼容的 fetch stub 覆盖 LLM/VL JSON 重试与图片转录。GitHub Actions 每次 push / PR 自动跑（`.github/workflows/test.yml`）。

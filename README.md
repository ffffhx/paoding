...
给它一个做菜视频（B站 / YouTube 链接、本地文件；抖音直链在 `yt-dlp` 当前可抽取时可用），**或一段图文/文字帖**（小红书 / 公众号 / 任意粘贴的文字），庖丁会：
...
| 每步为什么 | ✅ 自动逐步解释 | ❌ | ❌ | ✅ 人工编辑 |
| 中文平台（B站/抖音/小红书） | ✅ 重点支持 | ❌ | ❌ | ❌ |
| 菜谱集 / 收藏夹 | ✅ 多菜谱集，详情页勾选归档 | ✅ | ✅ 常见 | ⚠️ 内容收藏为主 |
| 食材库存 / 能做什么 | ✅ 本地覆盖率排序 + 缺料提示 + AI 思路 | ⚠️ 依部署/插件能力 | ✅ 常见 | ❌ |
| AI 菜谱变体 | ✅ 素食化 / 更健康 / 低盐 / 替换食材，落库为新菜谱 | ⚠️ 取决于集成 | ✅ 部分商业 App 支持 | ❌ 人工内容为主 |
| 移动端跟做体验 | ✅ PWA/APK、暗色、触控、焦点态和打印细化 | ✅ Web 为主 | ✅ 原生 App 强 | ⚠️ 内容阅读体验强 |
...
| | |
|---|---|
| 🎬 **智能解析** | 粘链接（B站/YouTube；抖音直链依赖 `yt-dlp` 当前支持）或传视频，**实时进度 + 排队**、可后台化；手机可从别的 App 分享链接直接解析 |
| 🔎 **读画面字幕/配方卡** | 可选视觉 OCR（qwen2.5-VL）：场景切换抽帧 + 片头/片尾配方卡专项捕获（总帧数仍受上限控制），逐字转录屏上字幕/完整配料表，**兜住没口播、只有字幕或片尾配方卡的视频** |
...
| 📖 **跟做模式** | 一步一屏 / 大字 / 屏幕常亮 / 进度条 / 左右滑 / 桌面键盘快捷键 / 断点续做 / **本步用到的食材高亮** |
...
| 🪄 **AI 菜谱变体** | 详情页一键生成素食化 / 更健康（少油少糖）/ 低盐 / 替换指定食材版本；变体落库成新菜谱，标明原菜谱回链与「AI 改编，未经实做验证」 |
| 📚 **菜谱集** | 创建早餐 / 宴客 / 减脂等菜谱集；一道菜可加入多个集，列表封面取集内第一道菜图片，删除集不删菜谱 |
| 🧁 **工具/器具清单** | 甜品/烘焙重点列出打蛋器、裱花袋、抹刀、模具等关键工具；有替代品说明代价，无替代明确写原因，推断项单独标注 |
| 🧺 **食材 & 购物** | 份量缩放（按 `qty/unit` 精确重算）+ 中餐单位速查；我的食材库存（名称+数量备注、常用 chips）按覆盖率回答「能做什么」，可忽略“适量/少许”调料；加入购物清单时库存已有项自动标「已有」；购物清单同名合并 + 按超市货架分区，本周计划支持每日营养合计、周日均和多菜同做时间线 |
| ✏️ **可编辑** | AI 出错能直接改：标题/用量/步骤/讲解随手修正，食材/步骤可增删和上移下移，标签可直接编辑，保存即同步 |
| 🏠 **多用户** | `PAODING_API_TOKENS` 支持家庭多用户：菜谱共享，收藏/笔记/评分/购物清单/菜谱集/食材库存/本周计划/最近任务按 token 隔离 |
| ☁️ **同步 & 备份** | 收藏/笔记/评分/购物清单/菜谱集/食材库存**跨设备共享**（手机↔电脑）；一键导出备份 / 导入恢复，服务端自动定期备份 |
| 🔄 **生态互通** | 导入单个 **schema.org Recipe JSON-LD**；复制 Markdown、下载 **Cooklang `.cook`** 与 **schema.org JSON-LD**（含标准 HowTo `tool`）；菜谱分享图卡 PNG / 系统分享；详情页一键打印/PDF（保留标题、来源、食材、工具、步骤和每步 why）；公开分享页可导出 JSON-LD，方便复制到自己的庖丁实例 |
| ⭐ **收藏 & 记录** | 收藏整菜 + 收藏单步技巧、笔记、做过打卡 + 评分；我的厨房统计本周/本月/连续做饭和常做菜；菜谱库支持搜索、标签/食材/快捷筛选和按最近/评分/时长/名称排序 |
| 🌙 **顺手** | 暗色模式、字号、朗读语速、装到主屏（PWA）、离线看已解析菜谱、双指缩放；日期/时间/数字按界面语言本地化；统一 design tokens、hover/active/focus-visible、骨架加载和 reduced-motion 处理 |
| 🛡 **诚实** | 视频没讲清的绝不臆造，如实标「视频未明确」；每步带置信度，靠推测的会标「⚠️ 推测」；语音转写同音字只在烹饪语境明确时自动纠正并在 note 留原词，模糊用量给「参考：…（常识推测）」而不装成精确值 |
...
> 抖音支持受 `yt-dlp` 与平台反爬状态影响：公开视频直链可能可用，短链、搜索页、登录墙或风控页可能只返回站点壳。失败时请上传本地录屏/下载后的视频，或复制帖子正文到「粘贴文字」解析。
...
ollama pull qwen2.5vl:7b   # 可选：视觉模型（场景切换 + 片头/片尾配方卡捕获 + 每步状态图/食材图）
...
可选：`PAODING_OUTPUT_LANG=en` 会要求 LLM 用英文生成结构化菜谱、每步 why、营养估算、技法归纳和 AI 助手回答；默认 `zh` 不追加任何语言约束，保持现有中文输出行为。前端界面语言在 App 设置页切换，日期、时间和数字会按当前语言格式化。
...
1. **想用庖丁的人**：自托管自己的实例，按上面的本地或 Docker 方案启动即可。`PAODING_API_TOKEN` 是每个实例自己生成的私有凭证，只写在自己的 `.env` 或运行环境里，**不在代码仓库里**。
2. **实例主人给家人朋友开权限**：用 `PAODING_API_TOKENS="ming:tokenA,hua:tokenB"` 按人发放命名 token。菜谱库共享；收藏、笔记、评分、购物清单、菜谱集、食材库存、本周计划和最近任务按 token 隔离。删掉某人的 token 并重启服务，就撤销了这个人的访问。
3. **不建议把私有实例开给陌生人**：本地模型和家用机器算力有限，并发上限、排队和限流只是防滥用兜底，不是公开服务能力。公开分享菜谱请用只读分享页 `/r/`；想让对方解析和管理自己的菜谱，更适合让对方自托管。
...
- [x] 场景切换抽帧 + 片头/片尾配方卡专项捕获 + 视觉 OCR（qwen2.5-VL），兜住「没口播只有字幕/片尾配方卡」的视频（总帧数仍受 `max` 上限控制，前端「读画面字幕」开关）
...
- [x] 英文 UI 分块覆盖：设置语言、设置页其它静态项、首页/列表、菜谱详情、跟做模式、购物清单/计划、技法页、安装横幅、标签编辑弹窗、导出文件正文、toast 与错误提示
- [x] 工具/器具清单：甜品工具重点、替代方案/无替代理由、老菜谱 AI 补齐、编辑/跟做/打印/JSON-LD 互通
- [x] 菜谱集 Cookbooks：多集合归档、详情页加入/移出、per-user 同步
- [x] 食材库存 Pantry v1：库存 chips、能做什么覆盖率排序、AI 推荐、购物清单已有标记
- [x] AI 菜谱变体：素食化 / 更健康 / 低盐 / 替换食材，落库为新菜谱并回链原菜谱
- [x] UI 样式系统打磨：tokens、交互态、动效、暗色和打印回归
- [x] 菜谱分享图卡：详情页生成 PNG，支持系统分享或下载
- [x] 我的厨房统计：本周/本月/连续做饭、收藏、掌握技法和常做菜本地汇总
- [x] 桌面跟做快捷键：翻步、暂停/继续计时器、朗读和快捷键帮助
- [x] 日期、时间和数字格式 locale 化（统计、计划短日期、最近任务时间等按当前语言展示）
- [x] 更多真实视频调 prompt，扩充技法词表与命中质量（第十九批覆盖凉拌/汤/面点/卤味；早餐/甜品因 YouTube 403 与格式不可用记录为遗留；技法词表扩至 44 项）
